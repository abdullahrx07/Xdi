...
function getPrefix(threadID) {
	...
	// Use the numeric prefix as the lookup key — that is what gets stored in DB.
	const isJID = typeof threadID === "string" && threadID.includes("@");
	const lookupID = isJID ? String(threadID).split("@")[0].split(":")[0] : String(threadID);
	if (!isJID && isNaN(threadID))
		throw new Error('The first argument (threadID) must be a number');
	let prefix = global.GoatBot.config.prefix;
	const threadData = global.db.allThreadData.find(
		t => t.threadID == lookupID || t.threadID == String(threadID)
	);
	if (threadData)
...
function message(api, event) {
	...
	// E2EE JID threadIDs (e.g. "123456:69@msgr") — used to log which path failed.
	...
	return {
		send: async (form, callback) => {
			try {
				...
				log.err("MESSAGE_SEND", `send() failed for thread ${event.threadID}${isE2EEThread ? " (E2EE)" : ""}`, err);
		...
		reply: async (form, callback) => {
			try {
				...
				log.err("MESSAGE_REPLY", `reply() failed for thread ${event.threadID}${isE2EEThread ? " (E2EE)" : ""}, messageID=${event.messageID}`, err);
				// ⚠️ FIX (E2EE group "prefix only" silently not replying):
				// api.sendMessage(form, threadID, callback, messageID) threads the
				// reply under event.messageID. For E2EE groups that messageID can be
				// in a shape the send path doesn't accept as a valid reply target
				// (unlike E2EE DMs / normal groups, which tolerate it), so the call
				// throws — and previously that throw was swallowed here unless the
				// error mentioned "spam", so the user saw nothing and no log existed.
				// Fallback: retry as a plain send (no reply-threading) so the message
				// still goes out even when reply-threading isn't supported.
				if (isE2EEThread) {
					try {
						global.statusAccountBot = 'good';
						return await api.sendMessage(form, event.threadID, callback);
						...
						log.err("MESSAGE_REPLY_FALLBACK", `plain send() fallback also failed for E2EE thread ${event.threadID}`, err2);
						if (JSON.stringify(err2).includes('spam')) {
							setErrorUptime();
							throw err2;
						}
					}
