...
function getHeaders(urlStr, options, ctx, customHeader) {
    options = options || {};
    var host;
    try { host = new URL(urlStr).host; } catch (_) { host = urlStr.replace("https://", "").split("/")[0]; }

    // Consistent fallback modern User-Agent (Chrome Windows)
    ...
    // Determine correct Client Hints based on active User-Agent
    ...
    // Determine if it is a main document navigation/page-load or an AJAX/API request
    var isNavigation = urlStr.endsWith(".com") ||
                       urlStr.endsWith(".com/") ||
                       urlStr.includes("checkpoint") ||
                       (!urlStr.includes("ajax") && !urlStr.includes("graphql") && !urlStr.includes("api") && !urlStr.includes("chat"));

    var fetchSite = "same-origin";
    var fetchMode = "cors";
    var fetchDest = "empty";
    var acceptHeader = "text/html,application/xhtml+xml,application/xml;q=0.9,application/json;q=0.8,*/*;q=0.7";
