...
// xdi-fca's E2EE bundle uses Web Streams/File APIs — ensure globals before require
// (Node 18 lacks ReadableStream as global; Node <20 lacks File/Blob)
...
async function startBot(_0x3cad9e) {
  ...
  // ─── FCA load log ────────────────────────────────────────────────────────
  // The single FCA implementation is the npm package @rxabdullah/xdi-fca
  // (built-in Labyrinth E2EE). Resolving from node_modules is expected.
  const fcaResolvedPath = require.resolve("@rxabdullah/xdi-fca");
  const fcaVersion = (() => { try { return JSON.parse(require("fs").readFileSync(require("path").join(process.cwd(), "node_modules", "@rxabdullah", "xdi-fca", "package.json"), "utf8")).version; } catch { return "unknown"; } })();
  log.warn("FCA", `Loaded FCA v${fcaVersion} (@rxabdullah/xdi-fca) from: ${fcaResolvedPath}`);
  ...
  (function _0x3592ba(_0x812929) {
    ...
    let _0x54729a = false;
    // Build xdi-fca login options: config.optionsFca is the base, the
    // top-level config.e2ee block overrides E2EE settings. autoUpdate must
    // stay false — xdi-fca's built-in updater runs npm install + restarts the
    // process at runtime (code 2), which is unpredictable.
    const e2eeCfg = global.GoatBot.config.e2ee || {};
    const fcaOptions = Object.assign({}, global.GoatBot.config.optionsFca, {
      autoUpdate: false,
      appStateFile: path.join(process.cwd(), "account.txt"),
      enableE2EE: e2eeCfg.enable === true,
      e2eeMemoryOnly: e2eeCfg.memoryOnly !== false,
      e2eeDevicePath: e2eeCfg.memoryOnly === false && e2eeCfg.devicePath
        ? path.resolve(process.cwd(), e2eeCfg.devicePath)
        : undefined
    });
    login({
      'appState': _0x812929
    }, fcaOptions, async function (_0x3f689f, _0x4d5048) {
      if (_0x3f689f) {
      ...
      // ✅ E2EE — Labyrinth encrypted chats (xdi-fca callback API).
      // Skipped entirely when config.e2ee.enable === false.
      const e2eeCfgCb = global.GoatBot.config.e2ee || {};
      if (e2eeCfgCb.enable === true) {
        try {
          if (typeof _0x4d5048.connectE2EE === "function") {
            // If persistence is configured (e2ee.memoryOnly:false + e2ee.devicePath),
            // make sure the parent folder exists first —
            // the client may not create missing parent directories, which would
            // silently fall back to memory-only keys with no error surfaced.
            if (e2eeCfgCb.memoryOnly === false && e2eeCfgCb.devicePath) {
              const devicePathAbs = path.isAbsolute(e2eeCfgCb.devicePath)
                ? e2eeCfgCb.devicePath
                : path.join(process.cwd(), e2eeCfgCb.devicePath);
              fs.ensureDirSync(path.dirname(devicePathAbs));
            }
            const persistMode = (e2eeCfgCb.memoryOnly === false && e2eeCfgCb.devicePath)
              ? `persisted → ${e2eeCfgCb.devicePath}`
              : "memory-only (keys regenerate on restart)";
            // ⚠️ FIX: previously this callback only logged "e2ee_fully_ready" and
            // dropped every other event (e2ee_message, message_reply, e2ee_message_reaction,
            // etc.) on the floor — so handlerAction.js's E2EE cases (which are fully
            // implemented) were never reached and the bot never replied in E2EE
            // inbox/group threads. E2EE delivers all events through THIS callback
            // (not through listenMqtt's callback), so we must forward them into the
            // same dispatcher (`_0x290401`) that normal messages go through.
            // `_0x290401` is defined later in this same function via a hoisted
            // function declaration, so referencing it here (inside a callback that
            // only actually runs once events start arriving) is safe.
            _0x4d5048.connectE2EE((e2eeErr, event) => {
              if (e2eeErr) {
                log.warn("E2EE", "Error: " + (e2eeErr.message || e2eeErr));
                return;
              }
              if (!event) return;
              if (event.type === "e2ee_fully_ready") {
                log.info("E2EE", "Labyrinth client ready ✅ (" + persistMode + ")");
                return;
              }
              // Lifecycle-only events (e2ee_ready/e2ee_connected/e2ee_disconnected/
              // e2ee_device_data_changed) are still forwarded because handlerAction.js
              // already has explicit no-op cases for them — keeping ALL E2EE events
              // going through one path avoids re-introducing a second silent drop.
              try {
                _0x290401(null, event);
              } catch (dispatchErr) {
                log.err("E2EE", "Failed to dispatch E2EE event to handler: " + (dispatchErr.message || dispatchErr));
              }
            });
          }
        } catch (e2eeErr) {
      ...
      async function _0x290401(_0x570bf7, _0xb100c2) {
        ...
        // ⚠️ FIX (double response): this used to only cover type=='message', so
        // 'message_reply' and 'e2ee_message' (E2EE) got zero dedup — any duplicate
        // delivery during the restartListenMqtt overlap window (two mqtt listeners
        // briefly both active) produced a duplicate bot reply with no protection at
        // all for those two types. Also, on a duplicate it previously only
        // neutralized OLDER stale listener callbacks but let THIS current duplicate
        // still fall through to handlerAction below — guaranteeing one duplicate
        // reply per overlap, every time. Now: cover all three message-producing
        // types, and actually `return` on a detected duplicate instead of just
        // silencing future ones.
        const _0xDedupTypes = ['message', 'message_reply', 'e2ee_message'];
        if (_0xb100c2.messageID && _0xDedupTypes.includes(_0xb100c2.type)) {
          if (storage5Message.includes(_0xb100c2.messageID)) {
            ...
            log.warn("DEDUP", `Duplicate ${_0xb100c2.type} messageID=${_0xb100c2.messageID} ignored (listener overlap)`);
            return;
          } else {
            storage5Message.push(_0xb100c2.messageID);
          }
          if (storage5Message.length > 0x14) {
      ...
      if (_0x1c9406.enable == true) {
        ...
        const _0x4091cc = setInterval(async function () {
          ...
          try {
            await stopListening();
            // ⚠️ FIX (double response): this used to always sleep a hardcoded 1000ms
            // regardless of config, so config.json's restartListenMqtt.delayAfterStopListening
            // (meant to give the old mqtt connection enough time to fully close before a
            // new one opens) was defined but never actually applied — the real gap stayed
            // 1s no matter what value was configured, keeping the double-listener overlap
            // window wider than intended. Now the configured value is honored, with 1000ms
            // as a safe fallback if it's missing/invalid.
            const _0xStopDelay = Number.isFinite(_0x1c9406.delayAfterStopListening)
              ? _0x1c9406.delayAfterStopListening
              : 0x3e8;
            await sleep(_0xStopDelay);
            global.GoatBot.Listening = _0x4d5048.listenMqtt(_0xb703d8());
