...
function isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, lang) {
	...
	// Guard: userData may be null for E2EE users whose DB creation failed
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
	return async function (event, message) {
		...
		// E2EE JIDs (e.g. "123456:69@msgr") contain "@" — they are valid but fail
		// isNaN(), so we allow them explicitly alongside normal numeric IDs.
		const isE2EEThread  = typeof threadID === "string" && threadID.includes("@");
		const isE2EESender  = typeof senderID === "string" && senderID.includes("@");
		// For E2EE threads, use the numeric prefix as the DB key so we don't
		// create a duplicate row for the same underlying thread.
		const numericThreadID = isE2EEThread
			? threadID.split("@")[0].split(":")[0]
			: threadID;
		const numericSenderID = isE2EESender
			? senderID.split("@")[0].split(":")[0]
			: senderID;

		let threadData = global.db.allThreadData.find(t => t.threadID == threadID)
			|| (isE2EEThread && global.db.allThreadData.find(t => t.threadID == numericThreadID));
		let userData = global.db.allUserData.find(u => u.userID == senderID)
			|| (isE2EESender && global.db.allUserData.find(u => u.userID == numericSenderID));

		if (!userData && !isNaN(numericSenderID))
			userData = await usersData.create(numericSenderID);

		if (!threadData && (!isNaN(numericThreadID) || isE2EEThread)) {
			if (global.temp.createThreadDataError.includes(threadID)
				|| global.temp.createThreadDataError.includes(numericThreadID))
				return;
			// ⚠️ FIX (E2EE group "prefix only" not replying / silent after 1st msg):
			// threadsData.create(id) with no 2nd arg falls back to a real GraphQL
			// api.getThreadInfo(id) call, which doesn't understand E2EE JIDs at all.
			// For E2EE DMs the numeric JID prefix happens to equal the other
			// person's real Facebook UID, so getThreadInfo() "accidentally" resolves
			// — but an E2EE GROUP's numeric JID prefix is a Labyrinth-only id that
			// GraphQL has never heard of, so it throws. Previously that throw was
			// never caught here, so this whole async function (and every event
			// handler it was about to return: onStart/onChat/onReply/...) died
			// silently — matching the exact "works in normal group, dead in E2EE
			// group" symptom. Now: build a local threadInfo for E2EE threads so no
			// network call happens, and guard the call so a failure can never take
			// the whole handler down again.
			try {
				...
				threadData = await threadsData.create(isE2EEThread ? numericThreadID : threadID, e2eeThreadInfo);
				global.db.receivedTheFirstMessage[threadID] = true;
			} catch (createErr) {
				if (createErr.name === "DATA_ALREADY_EXISTS") {
					threadData = global.db.allThreadData.find(t => t.threadID == numericThreadID || t.threadID == threadID);
				} else {
					log.err("DATABASE", `Failed to create thread data for ${threadID}`, createErr);
					return;
				}
			}
		}
			...
			if (
				...
				global.db.receivedTheFirstMessage[threadID] = true;
				// Don't call refreshInfo for E2EE threads — GraphQL doesn't understand JIDs
				if (!isE2EEThread)
					await threadsData.refreshInfo(threadID);
			}
		}

		// Null-guard: if threadData creation failed upstream, bail out here
		// rather than crashing on threadData.settings below.
		if (!threadData) return;

		if (typeof threadData.settings.hideNotiMessage == "object")
			hideNotiMessage = threadData.settings.hideNotiMessage;

		// E2EE JID senderIDs (e.g. "123456:69@msgr") won't match numeric entries
		// in config.adminBot or threadData.adminIDs — normalise to the numeric
		// prefix so all role/ban checks work correctly for E2EE users too.
		const effectiveSenderID = isE2EESender ? numericSenderID : senderID;

		const prefix = getPrefix(threadID);
		const role = getRole(threadData, effectiveSenderID);
		const parameters = {
		...
		*/
		...
		async function onStart() {
			...
			const isAdminBot = (global.GoatBot.config.adminBot || []).includes(effectiveSenderID);
			...
			if (isBannedOrOnlyAdmin(userData, threadData, effectiveSenderID, threadID, isGroup, commandName, message, langCode))
				return;
				if (!command) {
				if (!hideNotiMessage.prefixOnly && (!commandName || commandName.trim() === ""))
				...
				if (!hideNotiMessage.commandNotFound && commandName) {
					...
					// —— candidates: real command names + all aliases, deduped by their canonical command —— //
					const candidates = [
						...Array.from(GoatBot.commands.keys()).map(cmd => ({ display: cmd, canonical: cmd })),
						...Array.from(GoatBot.aliases.keys()).map(alias => ({ display: alias, canonical: GoatBot.aliases.get(alias) }))
					];
					...
					const scored = candidates.map(({ display, canonical }) => {
						const c = display.toLowerCase();
						const substringBonus = input.includes(c) || c.includes(input) ? -100 : 0;
						return { display, canonical, score: levenDist(input, c) + substringBonus };
					});
					scored.sort((a, b) => a.score - b.score);
					// —— keep only the best-scoring match per canonical command (avoid showing cmd + its own alias twice) —— //
					const seenCanonical = new Set();
					const deduped = [];
					for (const s of scored) {
						if (seenCanonical.has(s.canonical)) continue;
						seenCanonical.add(s.canonical);
						deduped.push(s);
					}
					const bestScore = deduped[0].score;
					const bullet = langCode === "bn" ? "•" : "↳";
					const top = deduped.filter(s => s.score <= bestScore + 1).slice(0, 3).map(s => `   ${bullet} ${prefix}${s.display}`);
			...
			try {
				...
				log.info("CALL COMMAND", `${commandName} | ${userData?.name || effectiveSenderID} | ${senderID} | ${threadID} | ${args.join(" ")}`);
			}
			catch (err) {
		...
		*/
		async function onChat() {
			// Skip onChat for pure E2EE events (non-reply); they are handled via
			// the e2ee_message / message_reply branch in handlerAction.
			if (event.isE2EE && event.type === "e2ee_message") return;
				...
				command.onChat({
					...
					.then(async (handler) => {
						if (typeof handler == "function") {
							...
							try {
								await handler();
								log.info("onChat", `${commandName} | ${userData?.name || effectiveSenderID} | ${senderID} | ${threadID} | ${args.join(" ")}`);
							}
							catch (err) {
				...
				command.onAnyEvent({
					...
					.then(async (handler) => {
						if (typeof handler == "function") {
							try {
								await handler();
								log.info("onAnyEvent", `${commandName} | ${senderID} | ${userData?.name || effectiveSenderID} | ${threadID}`);
							}
							catch (err) {
				...
				command.onFirstChat({
					...
					.then(async (handler) => {
						if (typeof handler == "function") {
							...
							try {
								await handler();
								log.info("onFirstChat", `${commandName} | ${userData?.name || effectiveSenderID} | ${senderID} | ${threadID} | ${args.join(" ")}`);
							}
							catch (err) {
								await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "errorOccurred2", time, commandName, removeHomeDir(err.stack ? err.stack.split("\n").slice(0, 5).join("\n") : JSON.stringify(err, null, 2))));
							}
		...
		*/
		async function onReply() {
			...
			try {
				...
				log.info("onReply", `${commandName} | ${userData?.name || effectiveSenderID} | ${senderID} | ${threadID} | ${args.join(" ")}`);
			}
			catch (err) {
		...
		*/
		async function onReaction() {
			...
			const reactionDebug = !!(global.GoatBot.config && global.GoatBot.config.reactionDebug);
			const dispatchStartedAt = Date.now();
			if (reactionDebug) {
				log.info("reactionDebug", `dispatch start | messageID=${messageID} | senderID=${senderID} | reaction=${event.reaction} | onReaction.size=${onReaction.size}`);
			}
			const Reaction = onReaction.get(messageID);
			if (!Reaction) {
				if (reactionDebug) {
					log.info("reactionDebug", `no handler registered for messageID=${messageID} (this is normal if nothing is waiting on a reaction for this message)`);
				}
				return;
			}
			if (reactionDebug) {
				log.info("reactionDebug", `handler found for messageID=${messageID} | commandName=${Reaction.commandName || "(missing)"} | age=${Reaction.__createdAt ? (Date.now() - Reaction.__createdAt) + "ms" : "unknown"}`);
			}
			...
			try {
				...
				log.info("onReaction", `${commandName} | ${userData?.name || effectiveSenderID} | ${senderID} | ${threadID} | ${event.reaction}`);
				if (reactionDebug) {
					log.info("reactionDebug", `dispatch success | commandName=${commandName} | latency=${Date.now() - dispatchStartedAt}ms`);
				}
			}
			catch (err) {
				log.err("onReaction", `An error occurred when calling the command onReaction ${commandName}`, err);
				if (reactionDebug) {
					log.info("reactionDebug", `dispatch failed | commandName=${commandName} | latency=${Date.now() - dispatchStartedAt}ms | error=${err && err.message}`);
				}
		...
		async function handlerEvent() {
			...
			for (const [key] of allEventCommand) {
				...
				try {
					...
					if (typeof handler == "function") {
						await handler();
						log.info("EVENT COMMAND", `Event: ${commandName} | ${author} | ${userData?.name || effectiveSenderID} | ${threadID}`);
					}
				}
				catch (err) {
				...
				command.onEvent({
					...
					.then(async (handler) => {
						if (typeof handler == "function") {
							try {
								await handler();
								log.info("onEvent", `${commandName} | ${author} | ${userData?.name || effectiveSenderID} | ${threadID}`);
							}
							catch (err) {
